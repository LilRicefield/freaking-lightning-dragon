...
public class DragonStateManager {
    ...
    /**
     * Begins lightning attack sequence
     */
    public void beginLightningAttack(Vec3 targetPos) {
        setAttacking(true);
        setLightningTargetVec((float) targetPos.x, (float) targetPos.y, (float) targetPos.z);
        setHasLightningTarget(true);
        setLightningStreamActive(true);
        setLightningStreamProgress(0.0f);
    }

    /**
     * Ends lightning attack sequence
     */
    public void endLightningAttack() {
        setAttacking(false);
        setHasLightningTarget(false);
        setLightningStreamActive(false);
        setLightningStreamProgress(0.0f);
    }
    ...
    public void updateBankingFromRotation(float currentYaw, float prevYaw) {
        ...
        // Calculate banking angle from rotation difference (EntityNaga approach)
        float rotationDiff = currentYaw - prevYaw;
        float targetBanking = rotationDiff * 2.0f; // Convert rotation rate to banking angle
        
        // Use DragonMathUtil.approachDegreesSmooth for smooth banking transition
