...
public class DragonCombatManager {
    ...
    private int lightningBurstCooldown = 0; // Separate cooldown for Lightning Burst
    ...
    /**
     * Checks if Lightning Burst can be used (separate cooldown)
     */
    public boolean canUseLightningBurst() {
        return lightningBurstCooldown <= 0 && (activeAbility == null || activeAbility.canCancelActiveAbility());
    }
    ...
    public void sendAbilityMessage(AbilityType<LightningDragonEntity, ?> abilityType) {
        // Check cooldowns - Lightning Burst has separate cooldown system
        boolean canUse = false;
        if (abilityType == LightningDragonEntity.LIGHTNING_BURST_ABILITY) {
            canUse = canUseLightningBurst();
            System.out.println("DragonCombatManager: Lightning Burst cooldown: " + lightningBurstCooldown);
        } else {
            canUse = canUseAbility();
            System.out.println("DragonCombatManager: Normal ability cooldown: " + abilityCooldown);
        }
        
        System.out.println("DragonCombatManager: Trying to use ability: " + abilityType.getName());
        System.out.println("DragonCombatManager: Can use this ability: " + canUse);
        ...
        if (canUse) {
            ...
            System.out.println("DragonCombatManager: Created ability: " + newAbility.getClass().getSimpleName());

            if (newAbility.tryAbility()) {
                System.out.println("DragonCombatManager: Ability tryAbility() returned true - ACTIVATING");
                System.out.println("✅ SUCCESS: " + abilityType.getName() + " ability is working!");
                setActiveAbility(newAbility);
                
                // Apply separate cooldown systems
                if (abilityType == LightningDragonEntity.LIGHTNING_BURST_ABILITY) {
                    lightningBurstCooldown = ULTIMATE_COOLDOWN_TICKS; // 60 seconds for Lightning Burst only
                    abilityCooldown = 0; // No regular cooldown
                } else {
                    abilityCooldown = 60; // 3 seconds for normal abilities (reduced from 60 to 3 seconds)
                    // Lightning Burst cooldown unaffected
                }
                ...
                if (!dragon.level().isClientSide) {
                    ...
                    System.out.println("DragonCombatManager: Sent network packet to clients");
                } else {
                    System.out.println("DragonCombatManager: Client side - not sending packet");
                }
            } else {
                System.out.println("DragonCombatManager: Ability tryAbility() returned FALSE - conditions not met");
            }
        } else {
            System.out.println("DragonCombatManager: Cannot use ability - on cooldown");
        }
    }
    ...
    public void tick() {
        ...
        if (activeAbility != null) {
            if (activeAbility.isUsing()) {
                ...
                boolean shouldFinish = activeAbility.isFinished();
                if (shouldFinish) {
                    System.out.println("DragonCombatManager: Ability finished, ending it NOW");
                    System.out.println("DragonCombatManager: Calling activeAbility.end()...");
                    activeAbility.end();
                    System.out.println("DragonCombatManager: Setting activeAbility = null...");
                    activeAbility = null;
                    System.out.println("DragonCombatManager: Ability ended successfully!");
                }
            } else {
                activeAbility = null;
            }
        ...
        // Lightning Burst cooldown management
        if (lightningBurstCooldown > 0) {
            lightningBurstCooldown--;
        }
    }
