...
public class DragonCombatManager {
    ...
    public boolean canUseAbility() {
        return abilityCooldown <= 0 && (activeAbility == null || (!activeAbility.isUsing() && activeAbility.canCancelActiveAbility()));
    }
    ...
    public void setActiveAbility(Ability<LightningDragonEntity> ability) {
        ...
        // Sync ability type to client for animation
        if (ability != null) {
            ability.start();
            // For lightning beam, sync charge animation immediately, then beam phase later
            if (ability.getAbilityType().getName().equals("lightning_beam")) {
                // Sync charge animation immediately
                dragon.setSyncedActiveAbilityType("beam_start");
            } else {
                // Sync other abilities immediately
                dragon.setSyncedActiveAbilityType(ability.getAbilityType().getName());
            }
        } else {
            // Clear synced ability when no ability is active
            dragon.setSyncedActiveAbilityType("");
        }
    }
