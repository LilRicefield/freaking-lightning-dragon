...
public class LightningDragonEntity extends TamableAnimal implements GeoEntity, FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    ...
    public void useRidingAbility(String abilityName) {
        ...
        AbilityType<LightningDragonEntity, ?> abilityType = switch (abilityName) {
            ...
            case "continuous_lightning" -> LIGHTNING_BEAM_ABILITY; // Use the enhanced lightning beam for continuous
            default -> null;
        };
    ...
    // Breathing fire state for continuous lightning abilities
    ...
    public void setControlState(byte controlState) {
        ...
        // Handle continuous lightning beam (bit 3)
        boolean shouldFireLightning = (controlState & 8) != 0;
        if (shouldFireLightning && !isBreathingFire()) {
            setBreathingFire(true);
        } else if (!shouldFireLightning && isBreathingFire()) {
    ...
    protected @NotNull Vec3 getRiddenInput(Player player, @NotNull Vec3 deltaIn) {
        ...
        if (this.isFlying()) {
            ...
            float y = (float) lookVec.y * 0.3F; // Gentle pitch influence
    ...
    public void riderShootFire() {
        ...
        if (this.getRandom().nextInt(5) == 0) {
            ...
            // TODO: Create EntityDragonLightningCharge projectile
