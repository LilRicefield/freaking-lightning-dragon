...
import com.leon.lightningdragon.entity.ability.DragonAbility;
import com.leon.lightningdragon.entity.ability.DragonAbilityType;
...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    ...
    // Position tracking array for beam_origin bone (like Mowzie's headPos system)
     ...
     */
    public DragonAnimationController getAnimationController() {
    ...
    private boolean shouldUseUltimate() {
        ...
        // TODO: Replace with new Dragon ability system - check if ultimate ability is on cooldown
    ...
    public Vec3 getMouthPosition() {
        ...
        // For aerial attacks, we want the beam to originate slightly forward and down from the head
        // when the dragon is flying and looking down at targets
    ...
    public void tick() {
        ...
        // Handle animation chaining for walk/run stops
        if (level().isClientSide && enhancedMovementController != null) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        // Use EnhancedAnimationController for the main movement controller to get advanced features
        com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController<LightningDragonEntity> movementController =
                new com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController<>(this, "movement", 8, animationController::handleMovementAnimation);
        
        // Store reference to avoid casting issues later
        ...
        // Enhanced keyframe-based sound effects with animation progress
        movementController.setSoundKeyframeHandler(state -> {
            ...
            switch (sound) {
                case "wing_flap" -> {
                    // Speed up wing flaps with flight speed
                    double flightSpeed = getCachedHorizontalSpeed();
                ...
                case "dragon_step" -> {
                    ...
                    float weight = 1.0f;
                    if (isRunning()) weight *= 1.2f;
                    if (isVehicle()) weight *= 1.1f;
                    if (getHealth() < getMaxHealth() * 0.5f) weight *= 0.8f; // Lighter when injured
                    
                    float volume = 0.7f * weight;
                    float pitch = (0.9f + getRandom().nextFloat() * 0.2f) / weight; // Lower pitch for heavier steps
                    this.playSound(ModSounds.DRAGON_STEP.get(), volume, pitch);
                }
                case "takeoff_whoosh" -> {
                    // Takeoff sound varies with urgency
                    float urgency = getTarget() != null ? 1.3f : 1.0f;
                    this.playSound(SoundEvents.ENDER_DRAGON_FLAP, urgency * 1.2f, 0.8f);
                }
                case "landing_thud" -> {
                    // Landing impact varies with flight speed and weight
                    double impactSpeed = Math.abs(getDeltaMovement().y);
                    float volume = (float) Math.max(0.8f, 1.0f + impactSpeed * 2.0f);
                    float pitch = (float) Math.max(0.7f, 1.0f - impactSpeed * 0.3f);
                    this.playSound(SoundEvents.GENERIC_EXPLODE, volume * 0.6f, pitch);
                }
            }
        });
        ...
        // Banking controller removed - now handled by bone manipulation in renderer
    }
    ...
    private PlayState flightBlendPredicate(AnimationState<LightningDragonEntity> state) {
        ...
        if (shouldUseAccelerationAnimation()) {
            state.setAndContinue(FLAP_SWITCH); // Actually use FLY_FORWARD!
        } else {
            ...
            if (flappingWeight > 0.5f) {
                state.setAndContinue(FLAP_SWITCH);  // Use actual FLY_FORWARD for active flight
            } else {
                state.setAndContinue(GLIDE_SWITCH);    // Use actual FLY_GLIDE for gliding
            }
        }
        return PlayState.CONTINUE;
    }
    ...
    // Banking predicate removed - now handled by bone manipulation in renderer
    //NO MORE PREDICATES
    ...
    private com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController<LightningDragonEntity> enhancedMovementController = null;
    
    /**
     * Set the enhanced movement controller reference (called during registerControllers)
     */
    private void setEnhancedMovementController(com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController<LightningDragonEntity> controller) {
    ...
    private void syncEnhancedAnimationsToPhysics() {
        ...
        try {
            ...
            // Speed up during lightning abilities
            if (hasLightningTarget() || isLightningStreamActive()) {
     ...
     */
    public com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController<LightningDragonEntity> getEnhancedController(String name) {
     ...
     */
    public void transitionToAnimation(RawAnimation animation, int transitionTicks, com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController.TransitionCurve curve) {
    ...
    public void transitionToAnimation(RawAnimation animation, int transitionTicks) {
        transitionToAnimation(animation, transitionTicks, com.leon.lightningdragon.client.model.enhanced.EnhancedAnimationController.TransitionCurve.EASE_IN_OUT);
    }
    ...
    /**
     * Check for movement state changes and trigger appropriate stop sequences
     * Uses persistence system to prevent snapping during temporary vertical movement
     */
    private void checkMovementStateChanges() {
        boolean rawWalking = isWalking();
        boolean rawRunning = isActuallyRunning();
        
        // Update persistence counters
        if (rawWalking) {
            walkingStateTicks = Math.min(walkingStateTicks + 1, MOVEMENT_STATE_PERSISTENCE * 2);
        } else {
            walkingStateTicks = Math.max(walkingStateTicks - 1, 0);
        }
        
        if (rawRunning) {
            runningStateTicks = Math.min(runningStateTicks + 1, MOVEMENT_STATE_PERSISTENCE * 2);
        } else {
            runningStateTicks = Math.max(runningStateTicks - 1, 0);
        }
        
        // Determine smooth movement states (maintain state for a few ticks during jumps)
        boolean isCurrentlyWalking = walkingStateTicks > MOVEMENT_STATE_PERSISTENCE / 2;
        boolean isCurrentlyRunning = runningStateTicks > MOVEMENT_STATE_PERSISTENCE / 2;
        
        // Walking stopped - start walk stop sequence
        if (wasWalking && !isCurrentlyWalking && !isFlying()) {
            enhancedMovementController.startWalkStopSequence();
        }
        
        // Running stopped - start run stop sequence  
        if (wasRunning && !isCurrentlyRunning && !isFlying()) {
            enhancedMovementController.startRunStopSequence();
        }
