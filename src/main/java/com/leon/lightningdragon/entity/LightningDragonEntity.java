...
public class LightningDragonEntity extends TamableAnimal implements GeoEntity, FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    ...
    // ===== COMMAND SYSTEM (Like DinosaurEntity/TremorzillaEntity) =====
    ...
    public byte getControlState() {
        return this.entityData.get(DATA_CONTROL_STATE);
    }
    
    public void setControlState(byte controlState) {
        this.entityData.set(DATA_CONTROL_STATE, controlState);
    }
    ...
    public Vec3 getDismountLocationForPassenger(LivingEntity passenger) {
        ...
        Vec3 dismountPos = this.position().add(direction.scale(2.0));
        return dismountPos;
    }
    ...
    private void handleFlyingTick() {
        ...
        // Auto-land when sitting (but NOT when being ridden)
    ...
    private boolean shouldTakeoff() {
        // When being ridden, always allow takeoff for the first few ticks
        if (this.isVehicle() && timeFlying < TAKEOFF_TIME_THRESHOLD) {
            return true;
        }
        // Normal AI takeoff conditions
    ...
    private void handleSimpleLanding() {
        if (!level().isClientSide) {
            ...
            if (landingTimer > 60 || this.onGround()) {
                ...
                // Reset landingFlag so the dragon can take off again when ridden
                this.landingFlag = false;
            }
        }
    }

    // ===== TRAVEL METHOD (Like TremorzillaEntity) =====
    @Override
    public void travel(@NotNull Vec3 motion) {
        ...
        // Riding logic - based on Tremorzilla approach
    ...
    public @NotNull InteractionResult mobInteract(Player player, @NotNull InteractionHand hand) {
        ...
        if (!this.isTame()) {
            ...
            if (player.equals(this.getOwner())) {
                // Command cycling - Shift+Right-click cycles through commands (like DinosaurEntity)
                if (canOwnerCommand(player) && itemstack.isEmpty() && hand == InteractionHand.MAIN_HAND) {
                    ...
                    // Send feedback message to player (like in DinosaurEntity)
                ...
                // Mounting - Right-click without shift (like TremorzillaEntity)
                    ...
                    if (!this.isVehicle()) {
                        ...
                        if (player.startRiding(this)) {
                            // Auto-takeoff when player mounts (like a flying mount should)
                            if (!isFlying()) {
                                // Reset flight state for fresh takeoff
                                this.timeFlying = 0;
                                this.landingFlag = false;
                                this.landingTimer = 0;
    ...
    // ===== KEYBIND USING MOUNT IMPLEMENTATION (Like TremorzillaEntity) =====
    @Override
    public void onKeyPacket(Entity keyPresser, int type) {
        if (keyPresser.isPassengerOfSameVehicle(this)) {
            // Key types based on Tremorzilla reference:
            // 2 = G key (beam attack)
            // 3 = Left mouse (melee attack)
            if (type == 2) {
                // G key - Lightning Beam ability (requires charged state like Tremorzilla)
    ...
    protected Vec3 getRiddenInput(Player player, Vec3 deltaIn) {
        // Based on Tremorzilla's movement approach
    ...
    protected float getRiddenSpeed(Player rider) {
        return (float) (this.getAttributeValue(Attributes.MOVEMENT_SPEED) * (this.isFlying() ? 1.2F : 1.0F));
    }
