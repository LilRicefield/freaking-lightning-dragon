...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    ...
    // Ice & Fire style mouth position calculation for beam_origin locator
    // Uses the real head position and adds the locator offset
    @Override
    public Vec3 getMouthPosition() {

        // Get the base head position using Ice & Fire's method
        Vec3 headPos = getHeadPosition();

        // Add the beam_origin locator offset: [0, 6.5, -15] relative to head bone
        // Convert from geo coordinates to world coordinates
        float scale = MODEL_SCALE;
        double locatorForward = (-15.0 / 16.0) * scale;  // -15 in geo file
        double locatorUp = (6.5 / 16.0) * scale;          // 6.5 in geo file (already included in head pos)

        // Apply the forward offset from head position using dragon's body rotation
        // Ice & Fire uses yBodyRot + 90 for coordinate system
        double offsetX = locatorForward * Mth.cos((float) ((yBodyRot + 90) * Math.PI / 180));
        double offsetY = locatorUp * 0.1; // Small vertical adjustment for mouth vs head center
        double offsetZ = locatorForward * Mth.sin((float) ((yBodyRot + 90) * Math.PI / 180));

        return headPos.add(offsetX, offsetY, offsetZ);
    }
    ...
    private PlayState headAbilityPredicate(AnimationState<LightningDragonEntity> state) {
        // Only play head-only animations for lightning beam ability
        if (getActiveAbility() != null && getActiveAbility().isUsing()) {
            AbilityType<LightningDragonEntity, ?> activeType = getActiveAbilityType();

            // Check if this is the lightning beam ability (head-only)
            if (activeType == LIGHTNING_BEAM_ABILITY) {
                state.setAndContinue(LIGHTNING_BEAM);
                return PlayState.CONTINUE;
            }
        }

        // No head-only ability active, stop this controller
