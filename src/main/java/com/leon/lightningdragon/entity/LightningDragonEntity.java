...
//Just everything  
public class LightningDragonEntity extends TamableAnimal implements GeoEntity, FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    // Cache wrapper class
    ...
    // ===== MULTI-PART HITBOX SYSTEM =====
    // Tail segments (10 parts based on geometry file)
    public final DragonPartEntity tailPart1;
    public final DragonPartEntity tailPart2;
    public final DragonPartEntity tailPart3;
    public final DragonPartEntity tailPart4;
    public final DragonPartEntity tailPart5;
    public final DragonPartEntity tailPart6;
    public final DragonPartEntity tailPart7;
    public final DragonPartEntity tailPart8;
    public final DragonPartEntity tailPart9;
    public final DragonPartEntity tailPart10;
    
    // Wing parts
    public final DragonPartEntity leftWingPart;
    public final DragonPartEntity rightWingPart;
    
    // Head/Neck part
    public final DragonPartEntity headPart;
    public final DragonPartEntity neckPart;
    
    // All parts array for easy iteration
    public final DragonPartEntity[] allParts;
    ...
    public LightningDragonEntity(EntityType<? extends TamableAnimal> type, Level level) {
        ...
        // Initialize multi-part hitbox system
        // Tail segments - progressively smaller sizes based on dragon anatomy
        this.tailPart1 = new DragonPartEntity(this, "tail1", 3.0F, 2.5F);
        this.tailPart2 = new DragonPartEntity(this, this.tailPart1, "tail2", 2.8F, 2.3F);
        this.tailPart3 = new DragonPartEntity(this, this.tailPart2, "tail3", 2.5F, 2.0F);
        this.tailPart4 = new DragonPartEntity(this, this.tailPart3, "tail4", 2.3F, 1.8F);
        this.tailPart5 = new DragonPartEntity(this, this.tailPart4, "tail5", 2.0F, 1.5F);
        this.tailPart6 = new DragonPartEntity(this, this.tailPart5, "tail6", 1.8F, 1.3F);
        this.tailPart7 = new DragonPartEntity(this, this.tailPart6, "tail7", 1.5F, 1.2F);
        this.tailPart8 = new DragonPartEntity(this, this.tailPart7, "tail8", 1.3F, 1.0F);
        this.tailPart9 = new DragonPartEntity(this, this.tailPart8, "tail9", 1.0F, 0.8F);
        this.tailPart10 = new DragonPartEntity(this, this.tailPart9, "tail10", 0.8F, 0.6F);

        // Wing parts - large hitboxes for aerial combat
        this.leftWingPart = new DragonPartEntity(this, "left_wing", 6.0F, 4.0F);
        this.rightWingPart = new DragonPartEntity(this, "right_wing", 6.0F, 4.0F);

        // Head and neck parts for precise head targeting
        this.headPart = new DragonPartEntity(this, "head", 2.5F, 2.0F);
        this.neckPart = new DragonPartEntity(this, "neck", 2.0F, 3.0F);

        // Create the all parts array for easy iteration
        this.allParts = new DragonPartEntity[]{
                tailPart1, tailPart2, tailPart3, tailPart4, tailPart5,
                tailPart6, tailPart7, tailPart8, tailPart9, tailPart10,
                leftWingPart, rightWingPart, headPart, neckPart
        };
    }
    ...
    public void useRidingAbility(String abilityName) {
        if (!isTame() || getRidingPlayer() == null) return;
    ...
    public void requestRiderTakeoff() {
        if (!isTame() || getRidingPlayer() == null || isFlying()) return;
    ...
    public void setControlState(byte controlState) {
        ...
        setGoingUp((controlState & 1) != 0);
        setGoingDown((controlState & 2) != 0);
        setRiderAttacking((controlState & 4) != 0);
        ...
        if (yKeyHeld) {
            ...
            if (combatManager.getActiveAbility() == null || 
                !(combatManager.getActiveAbility() instanceof com.leon.lightningdragon.ai.abilities.combat.EnhancedLightningBeamAbility)) {
                combatManager.sendAbilityMessage(LIGHTNING_BEAM_ABILITY);
            }
        }
        // Note: The fireStopTicks countdown and ability ending is handled in updateLightningBeamFireStopLogic()
    }
    ...
    protected void positionRider(@NotNull Entity passenger, Entity.@NotNull MoveFunction moveFunction) {
        if (this.hasPassenger(passenger)) {
            ...
            double offsetY = this.getPassengersRidingOffset();
            Vec3 ridingPosition = this.position().add(0, offsetY, 0);
    ...
    public @NotNull Vec3 getDismountLocationForPassenger(@NotNull LivingEntity passenger) {
        ...
        return this.position().add(direction.scale(2.0));
    }
    ...
    public void tick() {
        ...
        // Update multi-part hitbox positions every tick
        if (!level().isClientSide) {
            updatePartPositions();
        }
        ...
        if (keyPresser.isPassengerOfSameVehicle(this)) {
            ...
            if (type == 2) {
                ...
                if (isTame() && !level().isClientSide) {
                    ...
                    useRidingAbility("lightning_beam");
                }
            }
            if (type == 3) {
                ...
                if (isTame() && !level().isClientSide) {
                    if (isFlying()) {
                        useRidingAbility("wing_lightning");
                    } else {
                        useRidingAbility("electric_bite");
                    }
                }
            }
        }
    ...
    protected @NotNull Vec3 getRiddenInput(Player player, @NotNull Vec3 deltaIn) {
        ...
        if (this.isFlying()) {
            ...
            float y = 0.0F;
            if (!this.isHovering() && combatManager.getActiveAbility() == null) {
    ...
    // ===== MULTI-PART HITBOX INTERFACE METHODS =====
    ...
    /**
     * Get a specific dragon part by name for abilities and targeting
     */
    public DragonPartEntity getDragonPart(String partName) {
        for (DragonPartEntity part : allParts) {
    ...
    /**
     * Dragon scale factor that affects part sizes
     */
    ...
    /**
     * Updates all multi-part hitbox positions based on dragon's current pose and animation
     */
    private void updatePartPositions() {
        float scale = getScale() * MODEL_SCALE;
        float bodyYaw = (float) Math.toRadians(this.yBodyRot);
        float headPitch = (float) Math.toRadians(this.getXRot());

        // Trigonometry calculations - cached for performance
        Vec3 bodyRotVec = getCachedBodyRotationVector();
        double cosYaw = bodyRotVec.z;
        double sinYaw = bodyRotVec.x;
        double cosPitch = Math.cos(headPitch);
        double sinPitch = Math.sin(headPitch);

        // === HEAD AND NECK POSITIONING ===
        // Head part - positioned at head bone location
        if (headPart != null) {
            // Head bone coordinates from geo file: [0, 6.75, -13.35]
            double headForward = (-13.35 / 16.0) * scale;
            double headUp = (6.75 / 16.0) * scale;

            double headX = this.getX() + sinYaw * headForward * cosPitch;
            double headY = this.getY() + headUp + headForward * sinPitch;
            double headZ = this.getZ() + cosYaw * headForward * cosPitch;

            headPart.setPosCenteredY(new Vec3(headX, headY, headZ));
            headPart.refreshDimensions();
        }

        // Neck part - positioned between body and head
        if (neckPart != null) {
            // Neck positioning at neck2 bone: [0, 5.5, -10.5]
            double neckForward = (-10.5 / 16.0) * scale;
            double neckUp = (5.5 / 16.0) * scale;

            double neckX = this.getX() + sinYaw * neckForward * cosPitch;
            double neckY = this.getY() + neckUp + neckForward * sinPitch;
            double neckZ = this.getZ() + cosYaw * neckForward * cosPitch;

            neckPart.setPosCenteredY(new Vec3(neckX, neckY, neckZ));
            neckPart.refreshDimensions();
        }

        // === WING POSITIONING ===
        // Left wing - positioned at leftwing bone: [-8, 4, -2]
        if (leftWingPart != null) {
            double wingSide = (-8.0 / 16.0) * scale;
            double wingUp = (4.0 / 16.0) * scale;
            double wingForward = (-2.0 / 16.0) * scale;

            // Transform wing position using body rotation
            double leftWingX = this.getX() + cosYaw * wingSide + sinYaw * wingForward;
            double leftWingY = this.getY() + wingUp;
            double leftWingZ = this.getZ() - sinYaw * wingSide + cosYaw * wingForward;

            leftWingPart.setPosCenteredY(new Vec3(leftWingX, leftWingY, leftWingZ));
            leftWingPart.refreshDimensions();
        }

        // Right wing - positioned at rightwing bone: [8, 4, -2]
        if (rightWingPart != null) {
            ...
            // Transform wing position using body rotation
            double rightWingX = this.getX() + cosYaw * wingSide + sinYaw * wingForward;
            double rightWingY = this.getY() + wingUp;
            double rightWingZ = this.getZ() - sinYaw * wingSide + cosYaw * wingForward;

            rightWingPart.setPosCenteredY(new Vec3(rightWingX, rightWingY, rightWingZ));
            rightWingPart.refreshDimensions();
        }

        // === TAIL POSITIONING ===
        // Position tail segments in a chain based on geometry file coordinates
        // Tail bone coordinates from geo file (10 segments)
        double[] tailPositions = {
            7.0, 13.0, 18.5, 23.5, 28.0, 32.0, 35.5, 38.5, 41.0, 43.0 // Z positions
        };
        
        DragonPartEntity[] tailParts = {
            tailPart1, tailPart2, tailPart3, tailPart4, tailPart5,
            tailPart6, tailPart7, tailPart8, tailPart9, tailPart10
        };

        for (int i = 0; i < tailParts.length; i++) {
            DragonPartEntity tailPart = tailParts[i];
            if (tailPart != null) {
                // Tail segments are behind the body (positive Z in local coordinates)
                double tailDistance = (tailPositions[i] / 16.0) * scale;
                double tailHeight = (2.0 / 16.0) * scale; // Slight vertical offset

                // Calculate chained tail motion for more realistic movement
                double chainOffset = Math.sin(this.tickCount * 0.05 + i * 0.3) * 0.2;
                
                // Position tail segment
                double tailX = this.getX() - sinYaw * tailDistance + cosYaw * chainOffset;
                double tailY = this.getY() + tailHeight;
                double tailZ = this.getZ() - cosYaw * tailDistance - sinYaw * chainOffset;

                tailPart.setPosCenteredY(new Vec3(tailX, tailY, tailZ));
                tailPart.refreshDimensions();
            }
        }
    }
    
    // ===== DRAGON FIRE/LIGHTNING SYSTEM =====
    ...
    public void stimulateFire(double burnX, double burnY, double burnZ, int syncType) {
        if (syncType == 1 && !level().isClientSide) {
            // Server->Client: Send fire sync to all clients
            // TODO: Send MessageDragonSyncFire to all players
        } else if (syncType == 2 && level().isClientSide) {
            // Client->Server: Send fire sync to server
            // TODO: Send MessageDragonSyncFire to server
        }
    ...
    public void riderShootFire() {
        ...
        if (rider == null || level().isClientSide) return;
        ...
        if (this.getRandom().nextInt(5) == 0) {
            ...
            double chargeX = lookVec.x;
            double chargeY = lookVec.y;
            double chargeZ = lookVec.z;
            

            // For now, just create lightning at target location
            Vec3 targetPos = getEyePosition().add(lookVec.scale(15.0));
            stimulateFire(targetPos.x, targetPos.y, targetPos.z, 1);
        } else {
            // Lightning stream (80% chance) - use ray tracing
            net.minecraft.world.phys.HitResult hitResult = rayTraceRider(rider, 10.0, 1.0F);
            if (hitResult != null) {
    ...
    private void createLightningEffects(double x, double y, double z) {
        ...
        level().playLocalSound(x, y, z, SoundEvents.LIGHTNING_BOLT_IMPACT, 
            SoundSource.HOSTILE, 0.5F, 1.0F + random.nextFloat() * 0.2F, false);
    }
    ...
    private void applyLightningDamage(double x, double y, double z) {
        ...
        for (LivingEntity entity : entities) {
            if (entity != this && entity != getRidingPlayer()) {
                entity.hurt(this.damageSources().lightningBolt(), 8.0F);
    ...
    private void updateLightningBeamFireStopLogic() {
        ...
        if (isBreathingFire() && fireStopTicks <= 0) {
            ...
            if (combatManager.getActiveAbility() instanceof EnhancedLightningBeamAbility) {
                combatManager.forceEndActiveAbility();
            }
