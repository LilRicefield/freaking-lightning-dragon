...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob, KeybindUsingMount {
    ...
    public void tick() {
        ...
        // Update walk/run transition value (similar to ModelUmvuthana but simpler)
    ...
    private void updateGlideTransition() {
        ...
        // Get flapping amount from animation controller
        float flappingWeight = animationController.getFlappingFraction(1.0f);
        
        if (flappingWeight > 0.5f || shouldUseAccelerationAnimation()) {
            targetValue = 0.0f; // Full flap (not gliding)
        } else {
            targetValue = 1.0f; // Full glide
        }
        
        // Smooth transition like ModelUmvuthana (slightly slower for smoother flight transitions)
