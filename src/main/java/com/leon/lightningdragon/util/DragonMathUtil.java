...
public class DragonMathUtil {
    ...
    /**
     * Smooth degree-based approach for rotation values
     */
    ...
    /**
     * Create smooth banking based on turn rate (EntityNaga-style)
     * Use this for immediate banking calculation without smoothing
     */
    public static float calculateBanking(float currentYaw, float previousYaw, float maxBanking) {
        float yawDiff = Mth.wrapDegrees(currentYaw - previousYaw);
        // EntityNaga approach: direct yaw difference, clamped for safety
        return Mth.clamp(yawDiff, -maxBanking, maxBanking);
    }

    /**
     * EntityNaga-style banking with smoothing - preferred method
     * This handles the smooth interpolation that EntityNaga uses
     */
    public static float updateBankingSmooth(float currentBanking, float prevBanking, float currentYaw, float prevYaw) {
        float rotationDiff = currentYaw - prevYaw;
        // EntityNaga uses this exact pattern: approachDegreesSmooth with rotation difference
        return approachDegreesSmooth(currentBanking, prevBanking, rotationDiff, 0.5f, 0.1f);
    }
    ...
    public static void smoothLookAt(LivingEntity entity, Entity target, float maxYawChange, float maxPitchChange) {
        ...
        entity.setYRot(Mth.approachDegrees(entity.getYRot(), targetYaw, maxYawChange));
        entity.setXRot(Mth.approachDegrees(entity.getXRot(), targetPitch, maxPitchChange));
