...
public class EnhancedLightningBeamAbility extends Ability<LightningDragonEntity> {
    ...
    public boolean tryAbility() {
        // Debug passenger system
        System.out.println("EnhancedLightningBeamAbility DEBUG:");
        System.out.println("- isVehicle: " + getUser().isVehicle());
        System.out.println("- getFirstPassenger: " + getUser().getFirstPassenger());
        System.out.println("- isTame: " + getUser().isTame());
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player player) {
            System.out.println("- passenger is Player: " + player.getName().getString());
            System.out.println("- isOwnedBy passenger: " + getUser().isOwnedBy(player));
        }
        System.out.println("- getControllingPassenger: " + getUser().getControllingPassenger());
        System.out.println("- getRidingPlayer: " + getUser().getRidingPlayer());
        
        // TEMP FIX: Check if player is riding directly without ownership check
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player) {
            System.out.println("- TEMP FIX: Player is riding dragon, allowing ability");
            return true;
        }
        
        // Original check for comparison
    ...
    public void start() {
        ...
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player rider) {
            // Aim where the rider is looking
            Vec3 headPos = getUser().getHeadPosition();
            Vec3 lookVec = rider.getLookAngle();
            beamTarget = headPos.add(lookVec.scale(MAX_RANGE));
    ...
    private void chargePhase() {
        ...
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player rider) {
            // Continuously track where the rider is looking for responsive aiming
            Vec3 headPos = getUser().getHeadPosition();
            Vec3 lookVec = rider.getLookAngle();
            beamTarget = headPos.add(lookVec.scale(MAX_RANGE));
        } else if (getUser().getTarget() != null) {
    ...
    private void beamPhase() {
        ...
        // Continue targeting - either player look direction or target
    ...
    public boolean isFinished() {
        ...
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player) {
            ...
            // Debug control state
            System.out.println("EnhancedLightningBeamAbility isFinished() DEBUG:");
            System.out.println("- Control State: " + getUser().getControlState());
            System.out.println("- Bit 4 (Y key): " + stillPressingKey);
            System.out.println("- Ticks in use: " + getTicksInUse());
            
            if (!stillPressingKey) {
                // Key released, stop the beam
                System.out.println("- Y key released, stopping beam");
                return true;
            }
    ...
    @Override
    public void end() {
        System.out.println("EnhancedLightningBeamAbility: END() called - cleaning up immediately");
        super.end();
        ...
        burnProgress = 0;
        phase = 0;
        System.out.println("EnhancedLightningBeamAbility: END() completed - beam should stop now");
    }
