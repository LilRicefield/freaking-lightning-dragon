...
public class EnhancedLightningBeamAbility extends Ability<LightningDragonEntity> {
    ...
    private void beamPhase() {
        ...
        if (getUser().getRidingPlayer() != null && getTicksInUse() % 2 == 0) {
    ...
    public boolean isFinished() {
        ...
        if (getUser().getRidingPlayer() != null) {
            // Check if the rider is still holding the continuous lightning key (control state bit 3)
            boolean stillPressingKey = (getUser().getControlState() & 8) != 0;
