...
public class EnhancedLightningBeamAbility extends Ability<LightningDragonEntity> {
    ...
    public boolean tryAbility() {
        ...
        String currentAnim = getUser().getSyncedActiveAbilityType();
        if (!currentAnim.isEmpty()) {
            System.out.println("DEBUG - tryAbility called while animation active: " + currentAnim);
        }
        ...
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player) {
            System.out.println("DEBUG - Player riding dragon - beam ability available");
            return true;
        }
        
        // Check if tamed dragon has riding player
        if (getUser().getRidingPlayer() != null) {
            System.out.println("DEBUG - Tamed dragon has riding player - beam ability available");
            return true;
        }
        
        // Otherwise, require a target within specific range (AI-controlled)
        boolean result = getUser().getTarget() != null &&
                getUser().distanceToSqr(getUser().getTarget()) >= 256 && // 16+ blocks (no overlap with breath)
                getUser().distanceToSqr(getUser().getTarget()) <= 1024;  // up to 32 blocks
        
        if (result) {
            System.out.println("DEBUG - AI target in range - beam ability available");
        }
    ...
    public void start() {
        ...
        // Clear any lingering animation state from previous use
        String currentAnim = getUser().getSyncedActiveAbilityType();
        System.out.println("DEBUG - Starting new beam, current animation state: '" + currentAnim + "'");
        
        if ("beam_end".equals(currentAnim) || "lightning_beam".equals(currentAnim)) {
            System.out.println("DEBUG - FORCE clearing previous animation: " + currentAnim);
            getUser().setSyncedActiveAbilityType("");
        ...
        // IMMEDIATELY sync beam_start animation to client for proper sequence
        getUser().setSyncedActiveAbilityType("beam_start");
        System.out.println("DEBUG - Beam ability started - syncing beam_start animation to client");
    ...
    private void beamPhase() {
        if (phase == 0) {
            // Just entered beam phase - transition from beam_start to lightning_beam
            phase = 1;
            getUser().level().playSound(null, getUser().getX(), getUser().getY(), getUser().getZ(),
                    SoundEvents.LIGHTNING_BOLT_IMPACT, SoundSource.HOSTILE, 2.0f, 1.0f);
            
            // Transition to lightning_beam animation
            getUser().setSyncedActiveAbilityType("lightning_beam");
            System.out.println("DEBUG - Beam phase started - transitioning to lightning_beam animation");
        }
    ...
    public boolean isFinished() {
        ...
        if (getUser().getFirstPassenger() instanceof net.minecraft.world.entity.player.Player) {
            ...
            System.out.println("DEBUG - isFinished: ticks=" + getTicksInUse() + ", controlState=" + getUser().getControlState() + 
                             ", stillPressingY=" + stillPressingKey + ", phase=" + phase);
            
            if (!stillPressingKey) {
                // Key released, stop the beam
                System.out.println("DEBUG - Y key not held, ending ability");
                return true;
            }
    ...
    public void end() {
        ...
        // Only sync beam_end if we actually fired the beam (not if cancelled during charge)
        if (phase >= 1) {
            getUser().setSyncedActiveAbilityType("beam_end");
            System.out.println("DEBUG - Beam ending - syncing beam_end animation to client");
        } else {
            // If cancelled during charge, just clear the animation
            getUser().setSyncedActiveAbilityType("");
            System.out.println("DEBUG - Beam cancelled during charge - clearing animation");
        }
