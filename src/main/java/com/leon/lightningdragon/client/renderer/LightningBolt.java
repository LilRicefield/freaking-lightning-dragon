...
/**
 * PROFESSIONAL Lightning Bolt Data System - Ice & Fire Style
 * Advanced procedural lightning generation with realistic branching and animation
 */
public class LightningBolt {
    ...
    public LightningBolt(Vec3 start, Vec3 end) {
        this(BoltRenderInfo.DEFAULT, start, end, (int) (Math.sqrt(start.distanceTo(end) * 100)));
    }

    public LightningBolt(BoltRenderInfo info, Vec3 start, Vec3 end, int segments) {
    ...
    public LightningBolt count(int count) {
        this.count = count;
        return this;
    }

    public LightningBolt size(float size) {
        this.size = size;
        return this;
    }

    public LightningBolt spawn(SpawnFunction spawnFunction) {
        this.spawnFunction = spawnFunction;
        return this;
    }

    public LightningBolt fade(FadeFunction fadeFunction) {
        this.fadeFunction = fadeFunction;
        return this;
    }

    public LightningBolt lifespan(int lifespan) {
    ...
    public Vec3 getStartPos() { return start; }
    public Vec3 getEndPos() { return end; }
    ...
    public static class BoltRenderInfo {
        ...
        public static BoltRenderInfo electricity() {
            // Electric blue-purple like Ice & Fire Lightning Dragons
            return new BoltRenderInfo(0.5F, 0.25F, 0.25F, 0.15F, 
                    new Vector4f(0.70F, 0.45F, 0.89F, 0.9F), 0.8F);
        }

        public BoltRenderInfo() {
        }

        public BoltRenderInfo(float parallelNoise, float spreadFactor, float branchInitiationFactor, 
                              float branchContinuationFactor, Vector4f color, float closeness) {
