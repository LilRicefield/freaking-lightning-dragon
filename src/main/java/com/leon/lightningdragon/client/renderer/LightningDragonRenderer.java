...
import com.leon.lightningdragon.entity.DragonPartEntity;
...
public class LightningDragonRenderer extends GeoEntityRenderer<LightningDragonEntity> {
    ...
    public void render(@NotNull LightningDragonEntity entity, float entityYaw, float partialTick,
        ...
        // Check if debug2 hitboxes are enabled (F3 + B pressed)
        if (Minecraft.getInstance().getEntityRenderDispatcher().shouldRenderHitBoxes()) {
            renderMultipartDebugHitboxes(entity, poseStack, bufferSource, partialTick);
        }
    }
    /**
     * Renders debug2 hitboxes for all dragon parts when F3+B is active
     */
    private void renderMultipartDebugHitboxes(LightningDragonEntity entity, PoseStack poseStack,
                                              MultiBufferSource bufferSource, float partialTick) {

        for (DragonPartEntity part : entity.allParts) { // Make allParts public or add getter
            if (part != null) {
                poseStack.pushPose();

                // Get part position
                double partX = Mth.lerp(partialTick, part.xo, part.getX());
                double partY = Mth.lerp(partialTick, part.yo, part.getY());
                double partZ = Mth.lerp(partialTick, part.zo, part.getZ());

                // Translate to part position relative to dragon
                poseStack.translate(partX - entity.getX(), partY - entity.getY(), partZ - entity.getZ());

                // Get the part's bounding box
                AABB partBB = part.getBoundingBox();
                double width = partBB.getXsize();
                double height = partBB.getYsize();
                double depth = partBB.getZsize();

                // Render the debug2 outline - cyan color for dragon parts
                LevelRenderer.renderLineBox(poseStack,
                        bufferSource.getBuffer(RenderType.lines()),
                        -width/2, 0, -depth/2,     // Min coords
                        width/2, height, depth/2,   // Max coords
                        0.0f, 1.0f, 1.0f, 1.0f);   // Cyan color (RGBA)
