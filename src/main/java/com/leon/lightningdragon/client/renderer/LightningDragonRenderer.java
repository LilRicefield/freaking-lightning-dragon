...
public class LightningDragonRenderer extends GeoEntityRenderer<LightningDragonEntity> {
    ...
    public void render(@NotNull LightningDragonEntity entity, float entityYaw, float partialTick,
        ...
        // Check if debug hitboxes are enabled (F3 + B pressed)
        if (Minecraft.getInstance().getEntityRenderDispatcher().shouldRenderHitBoxes()) {
            renderMultipartDebugHitboxes(entity, poseStack, bufferSource, partialTick);
        }
    }
    /**
     * Renders debug hitboxes for all dragon parts when F3+B is active
     */
    private void renderMultipartDebugHitboxes(LightningDragonEntity entity, PoseStack poseStack,
        ...
        for (DragonPartEntity part : entity.allParts) { // Make allParts public or add getter
            if (part != null) {
                ...
                // Render the debug outline - cyan color for dragon parts
