...
import com.leon.lightningdragon.client.model.LightningBeamModel;
...
public class LightningBeamLayer extends GeoRenderLayer<LightningDragonEntity> {
    ...
    // Cap texture: separate from inner/outer beam layers
    private static final ResourceLocation CAP_TEX = ResourceLocation.fromNamespaceAndPath("lightningdragon", "textures/effects/lightning_beam_cap.png");
    ...
    // End-caps: rendered with LightningBeamModel at start/end
    ...
    private final LightningBeamModel capModel = new LightningBeamModel();
    ...
    public void render(@NotNull PoseStack poseStack, LightningDragonEntity animatable, BakedGeoModel bakedModel,
        ...
        // Render beam end-caps using model (muzzle + impact)
        renderBeamCaps(poseStack, bufferSource, packedLight, width + OUTER_BEAM_BONUS, length);
    ...
    private void renderBeamCaps(PoseStack poseStack,
                                MultiBufferSource source,
                                int packedLight,
                                float width,
                                float length) {
        // Render the dedicated cap model using the beam texture; 16x16 will be stretched but consistent
        float s = Math.max(0.001f, width / 11.0f);
        VertexConsumer capBuffer = source.getBuffer(RenderType.entityTranslucent(CAP_TEX));

        // Muzzle cap (slightly forward to avoid z-fighting with beam body)
        poseStack.pushPose();
        poseStack.translate(0.0F, -1.5F, 0.01F);
        poseStack.scale(s, s, s);
        capModel.renderToBuffer(poseStack, capBuffer, packedLight, OverlayTexture.NO_OVERLAY, 1f, 1f, 1f, (float) 0.85);
        poseStack.popPose();

        // Impact cap at the beam end (slightly back along -Z to avoid z-fight)
        poseStack.pushPose();
        poseStack.translate(0.0F, -1.5F, Math.max(0.0F, length - 0.01F));
        // Flip to face backwards along the beam
