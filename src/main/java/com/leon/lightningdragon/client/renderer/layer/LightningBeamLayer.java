...
/**
 * Lightning beam render layer using Tremorzilla's cylindrical beam approach.
...
public class LightningBeamLayer extends GeoRenderLayer<LightningDragonEntity> {
    ...
    private static final LightningBeamModel BEAM_END_MODEL = new LightningBeamModel();
    ...
    private static final float OUTER_BEAM_BONUS = 0.12F;      // Extra width for outer glow layer
    ...
    private static final float END_CAP_OFFSET = 1.5F;         // How far back from beam end to place the end cap
    ...
    public void render(@NotNull PoseStack poseStack, LightningDragonEntity animatable, BakedGeoModel bakedModel,
        ...
        // Calculate beam direction and prepare transformation like Tremorzilla
    ...
    private void renderBeam(LightningDragonEntity entity, PoseStack poseStack, MultiBufferSource source, float partialTicks, float width, float length, boolean inner) {
        ...
        if (inner) {
            vertices = 4;
            ResourceLocation resourceLocation = INNER_TEX;
            vertexconsumer = source.getBuffer(RenderType.entityTranslucent(resourceLocation));
            speed = INNER_SPEED_MULTIPLIER;
        } else {
            vertices = 8;
            ResourceLocation resourceLocation = OUTER_TEX;
            vertexconsumer = source.getBuffer(RenderType.entityTranslucent(resourceLocation));
        ...
        if(inner){
            VertexConsumer endVertexConsumer = source.getBuffer(RenderType.entityTranslucent(getEndBeamTexture(entity)));
            poseStack.pushPose();
            poseStack.translate(0, 0, length - END_CAP_OFFSET); // configurable end cap position
            poseStack.mulPose(Axis.ZP.rotationDegrees(45));
    ...
    private ResourceLocation getEndBeamTexture(LightningDragonEntity entity) {
