...
import com.leon.lightningdragon.common.network.MessageBeamStart;
...
public class DragonRideKeybinds {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        // While beaming, sync mouth position each tick for precise damage origin
        if (dragon.isBeaming()) {
            // Use the same math used by visuals as a baseline; this stays consistent with current frame
            float pt = mc.getFrameTime();
            net.minecraft.world.phys.Vec3 start = dragon.computeHeadMouthOrigin(pt);
            NetworkHandler.INSTANCE.send(PacketDistributor.SERVER.noArg(), new MessageBeamStart(dragon.getId(), start));
        }
        ...
        // Disabled: client->server beam start override; rely on server computeHeadMouthOrigin
    }
    ...
    private static void handleDragonControlState(LightningDragonEntity dragon, LocalPlayer player) {
        ...
        if (controlState != previousState) {
            ...
            // Echo locally so next tick can detect both press and release transitions
