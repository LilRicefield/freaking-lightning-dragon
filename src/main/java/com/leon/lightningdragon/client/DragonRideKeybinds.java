...
public class DragonRideKeybinds {
    ...
    public static final KeyMapping DRAGON_SUMMON = new KeyMapping(
            "key.lightningdragon.summon_storm",
            InputConstants.Type.KEYSYM,
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (roarDown && !wasRoarDown) {
            NetworkHandler.INSTANCE.send(PacketDistributor.SERVER.noArg(),
                    new MessageDragonRideInput(false, false, DragonRiderAction.ABILITY_USE, "roar", 0, 0, 0));
        }

        // Handle summon storm as a one-shot on key press
        if (summonDown && !wasSummonDown) {
            NetworkHandler.INSTANCE.send(PacketDistributor.SERVER.noArg(),
                    new MessageDragonRideInput(false, false, DragonRiderAction.ABILITY_USE, "summon_storm", 0, 0, 0));
        }
