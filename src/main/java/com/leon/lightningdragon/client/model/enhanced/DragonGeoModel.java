 ...
 * Based on Mowzie's Mobs architecture but enhanced for dragon-specific features
 */
public abstract class DragonGeoModel<T extends GeoAnimatable> extends GeoModel<T> {
    ...
    /**
     * Get all tail bones in order for physics chains
     * Searches for tail1 through tail10 by default
     */
    public GeoBone[] getTailChainBones() {
        return getTailChainBones(10);
    }
    
    /**
     * Get tail bones with custom maximum count
     */
    public GeoBone[] getTailChainBones(int maxTailSegments) {
        java.util.List<GeoBone> tailBones = new java.util.ArrayList<>();
        
        for (int i = 1; i <= maxTailSegments; i++) {
            Optional<GeoBone> bone = getBone("tail" + i);
            if (bone.isPresent()) {
                tailBones.add(bone.get());
            } else {
                break; // Stop at first missing segment
            }
        }
        
        return tailBones.toArray(new GeoBone[0]);
    }
