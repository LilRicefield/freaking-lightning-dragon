...
public class DragonAnimationController {
    ...
    public PlayState handleMovementAnimation(AnimationState<LightningDragonEntity> state) {
        // Only head-only abilities override movement - others play through separate controllers
        if (dragon.getActiveAbility() != null && dragon.getActiveAbility().isUsing()) {
            // Check if this is a head-only ability (lightning beam)
            if (dragon.getActiveAbilityType() == LightningDragonEntity.LIGHTNING_BEAM_ABILITY) {
                // Let the head controller handle this, continue with body movement
                // Do NOT return the ability predicate here - let movement continue
            } else {
                // Full-body abilities still override movement completely
