...
public class DragonPhysicsController {
    ...
    public PlayState handleMovementAnimation(AnimationState<LightningDragonEntity> state) {
        ...
        if (dragon.isInSittingPose() || dragon.getSitProgress() > 0.5f) {
            ...
            // Ensure short ascents finish a full forward flap cycle
            if (flapLockTicks > 0) {
                flapLockTicks--;
            ...
            if (syncedMode == 2) {
                // Stationary/hover: show gentle forward flapping
                state.getController().transitionLength(6);
                state.setAndContinue(LightningDragonEntity.FLY_FORWARD);
                return PlayState.CONTINUE;
            }
            ...
            if (syncedMode == 0) {
                state.getController().transitionLength(6);
                // Occasional visual maintenance flaps even during glide
    ...
    private void updatePhysicsEnvelopes() {
        ...
        if (dragon.isFlying() && !dragon.isLanding() && !dragon.isDodging()) {
            boolean riderCoast = dragon.getControllingPassenger() != null && !dragon.isGoingUp() && !dragon.isGoingDown() && Math.abs(dragon.getDeltaMovement().y) < 0.06;
            boolean glideDominant = (glideEnv.raw() >= flapEnv.raw() - 0.10f) && hoverEnv.raw() < 0.60f;
            double vy = dragon.getDeltaMovement().y;
            boolean slowOrSinking = vy < 0.10 && vy > -0.35;

            if (maintenanceFlapTicks > 0) {
                maintenanceFlapTicks--;
            } else if (maintenanceFlapCooldown > 0) {
                maintenanceFlapCooldown--;
            } else if ((glideDominant || riderCoast) && slowOrSinking) {
