...
public class HornGoreAbility extends DragonAbility<LightningDragonEntity> {
    ...
    protected void beginSection(DragonAbilitySection section) {
        ...
        if (section.sectionType == AbilitySectionType.STARTUP) {
            // Trigger gore animation if present
    ...
    public void tickUsing() {
        ...
        // Continue moving toward target during startup and active phases
        LightningDragonEntity dragon = getUser();
        LivingEntity target = dragon.getTarget();
        if (target != null && (section.sectionType == AbilitySectionType.STARTUP || section.sectionType == AbilitySectionType.ACTIVE)) {
            // Continue pursuit during ability - this prevents standing still
            double distance = dragon.distanceTo(target);
            if (distance > 3.0) { // Only move if not too close
                dragon.getNavigation().moveTo(target, 1.2);
            }
        }
        
        if (section.sectionType != AbilitySectionType.ACTIVE || didHit) return;

        LivingEntity foundTarget = findTarget();
        if (foundTarget != null) {
            applyGore(foundTarget);
            didHit = true;
        }
    }
    ...
    private void applyGore(LivingEntity target) {
        ...
        // Optional thud/impact effects could be added here later
