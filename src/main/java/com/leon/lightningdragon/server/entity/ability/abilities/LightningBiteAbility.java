...
public class LightningBiteAbility extends DragonAbility<LightningDragonEntity> {
    ...
    private static final double BITE_RANGE_RIDDEN = 6.0;
    // More forgiving cone so mild turn jitter doesn't cause whiffs
    private static final double BITE_ANGLE_DEG = 100.0; // half-angle of cone
    ...
    protected void beginSection(DragonAbilitySection section) {
        ...
        if (section.sectionType == AbilitySectionType.STARTUP) {
            ...
            // Use the registered trigger key to sync to clients
    ...
    public void tickUsing() {
        ...
        if (section.sectionType == AbilitySectionType.ACTIVE && !didHitThisActive) {
            ...
            if (primary == null) {
                ...
                primary = raycastTargetAlongMouth(ridden ? 6.5 : 5.5, ridden ? 1.5 : 1.0);
            }
            if (primary != null) {
                bitePrimary(primary);
                chainFrom(primary);
    ...
    private LivingEntity findPrimaryTarget() {
        ...
        AABB box = new AABB(mouth, mouth).inflate(effectiveRange, effectiveRange, effectiveRange);
        ...
        for (LivingEntity e : candidates) {
            ...
            boolean veryClose = distToAabb < (effectiveRange * 0.5);
            boolean goodAngle = dot >= cosLimit;
            if (ridden) {
                // When ridden, relax angle slightly to account for rider pitch/turning during animation
