...
public class DragonSoundHandler {
    ...
    public void handleAnimationSound(SoundKeyframeEvent<LightningDragonEntity> event) {
        if (dragon.isStayOrSitMuted()) return; // Suppress all sounds while staying/sitting
    ...
    public void handleSoundByName(String soundName) {
        ...
        if (key.startsWith("step")) { handleStepSound(key, null); return; }
        switch (key) {
    ...
    public void playVocal(String key) {
        ...
        if (window > 0 && !dragon.level().isClientSide) {
    ...
    private void handleStepSound(String key, String locator) {
        ...
        // Respect Blockbench spacing: 0.0 (step2/left), 0.6 (step1/right) when walking
        boolean walking = dragon.isWalking() && !dragon.isActuallyRunning() && !dragon.isFlying();
        long now = dragon.tickCount;
        if (key != null && key.endsWith("2")) {
            // step2: left foot
            if (walking) {
                // If step1 just played recently, no special delay is needed for step2
                int delay = requiredDelayTicks(now - lastStep1Tick, 0);
                if (delay > 0) {
                    pendingStep2 = new PendingStep("step2", locator, delay);
                } else {
                    actuallyPlayStep("step2", locator);
                }
            } else {
                actuallyPlayStep("step2", locator);
            }
            ...
            // step1: right foot
            if (walking) {
                int delay = requiredDelayTicks(now - lastStep2Tick, WALK_STEP_SEPARATION_TICKS);
                if (delay > 0) {
                    pendingStep1 = new PendingStep("step1", locator, delay);
                } else {
                    actuallyPlayStep("step1", locator);
                }
            } else {
                actuallyPlayStep("step1", locator);
            }
