...
import com.leon.lightningdragon.server.entity.controller.DragonPhysicsController;
import com.leon.lightningdragon.server.entity.base.DragonEntity;
import com.leon.lightningdragon.server.entity.handler.DragonCombatHandler;
import com.leon.lightningdragon.server.entity.controller.DragonFlightController;
import com.leon.lightningdragon.server.entity.handler.DragonInteractionHandler;
import com.leon.lightningdragon.server.entity.handler.DragonKeybindHandler;
import com.leon.lightningdragon.server.entity.controller.DragonRiderController;
...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob {
    ...
    public void setGroundMoveStateFromAI(int state) {
        if (!this.level().isClientSide) {
            ...
            if (this.entityData.get(DATA_GROUND_MOVE_STATE) != s) {
                ...
                com.leon.lightningdragon.common.network.NetworkHandler.INSTANCE.send(
                        net.minecraftforge.network.PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> this),
                        new com.leon.lightningdragon.common.network.MessageDragonAnimState(this.getId(), (byte) s, (byte) getEffectiveFlightMode())
                );
            }
        }
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide && (isSleeping() || sleepingEntering || sleepingExiting)) {
            ...
            // Flight mode sync for observers (derive deterministically while flying)
            int flightMode = -1;
            if (isFlying()) {
                ...
                if (isTakeoff()) {
                    ...
                    // Rider-focused flight mode (ignore hover fraction). Simple, deterministic.
                    if (isGoingUp() || yDelta > 0.02) flightMode = 1; // flap
                    else if (isGoingDown() || yDelta < -0.02) flightMode = 0; // glide
                    else flightMode = 0; // level coasting => glide
                } else {
                    // AI: use fractions with small bias
            ...
            if (changed && (lastBroadcastGroundState != moveState || lastBroadcastFlightMode != flightMode)) {
                ...
                com.leon.lightningdragon.common.network.NetworkHandler.INSTANCE.send(
                        net.minecraftforge.network.PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> this),
                        new com.leon.lightningdragon.common.network.MessageDragonAnimState(this.getId(), (byte) moveState, (byte) flightMode)
                );
            }
        ...
        if (!level().isClientSide && !(isSleeping() || sleepingEntering || sleepingExiting)) {
            ...
            if (isFlying()) {
                ...
                if (isTakeoff()) {
                    ...
                    if (isGoingUp() || yDelta > 0.02) flightMode = 1;
                    else if (isGoingDown() || yDelta < -0.02) flightMode = 0;
                    else flightMode = 0;
                } else {
                    float glide = getGlidingFraction();
                    float flap = getFlappingFraction();
    ...
    protected void playHurtSound(@NotNull net.minecraft.world.damagesource.DamageSource source) {
        ...
        if (!level().isClientSide) {
            this.tryActivateAbility(com.leon.lightningdragon.common.registry.ModAbilities.HURT);
        }
    ...
    protected void registerGoals() {
        ...
        // Merged flight decision into DragonAirCombatGoal
        this.goalSelector.addGoal(6, new DragonAirCombatGoal(this));      // Air combat + flight decision
        ...
        this.goalSelector.addGoal(1, new com.leon.lightningdragon.server.ai.goals.DragonSleepGoal(this));
    ...
    public boolean hurt(DamageSource damageSource, float amount) {
        ...
        if (!level().isClientSide && !dying) {
            ...
            if (remaining <= 0.0f) {
                ...
                this.tryActivateAbility(com.leon.lightningdragon.common.registry.ModAbilities.DIE);
                return true; // handled
            }
        }
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            if (this.random.nextFloat() < 0.7f) {
                if (this.random.nextBoolean()) {
                    server.sendParticles(new com.leon.lightningdragon.common.particle.LightningArcData(size),
                            pos.x, pos.y, pos.z, 1, dir.x, dir.y, dir.z, 0.0);
                } else {
                    server.sendParticles(new com.leon.lightningdragon.common.particle.LightningStormData(size),
                            pos.x, pos.y, pos.z, 1, dir.x, dir.y, dir.z, 0.0);
                }
            }
            pos = pos.add(step);
    ...
    protected void removePassenger(@NotNull Entity passenger) {
        ...
        if (!this.level().isClientSide) {
            ...
            com.leon.lightningdragon.common.network.NetworkHandler.INSTANCE.send(
                    net.minecraftforge.network.PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> this),
                    new com.leon.lightningdragon.common.network.MessageDragonAnimState(this.getId(), (byte) 0, (byte) getSyncedFlightMode())
            );
        }
    }
