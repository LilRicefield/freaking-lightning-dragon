 ...
 */
package com.leon.lightningdragon.server.entity.dragons;
...
import com.leon.lightningdragon.common.registry.AbilityRegistry;
...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob {
    ...
    // Controller switch animations removed — animation.json directly drives poses

    // ===== ABILITY SYSTEM =====
    // TODO: Define new DragonAbilityType abilities here

    // ===== BONE POSITION TRACKING =====
    // Position tracking for beam_origin bone
    public Vec3 beamOriginPos = null;
    // Cached rider anchor (e.g., Gecko bone "rider"), set client-side by renderer
    public Vec3 riderAnchorPos = null;
    ...
    static final EntityDataAccessor<Float> DATA_BEAM_START_Z =
            SynchedEntityData.defineId(LightningDragonEntity.class, EntityDataSerializers.FLOAT);
    // Removed legacy lightning-stream sync (Citadel-era) in favor of ability-driven visuals
    ...
    // Command system now defined in base DragonEntity
    ...
    static final EntityDataAccessor<Boolean> DATA_ACCELERATING =
            SynchedEntityData.defineId(LightningDragonEntity.class, EntityDataSerializers.BOOLEAN);
    // Removed unused control-state/active-ability data accessors
    ...
    // (Removed unused walking/running state persistence fields)
    ...
    // Suppress vanilla footsteps; we play timed steps from animation keyframes
    @Override
    ...
    protected void defineSynchedData() {
        ...
        // (Lightning-stream synced fields removed)
    ...
    // (sendAbilityMessage removed; abilities are triggered directly or via network handler)
    ...
    public Vec3 computeHeadMouthOrigin(float partialTicks) {
        ...
        double wz = lx * sy + lz * cy;
        double wy = ly;

        return new Vec3(x + wx, y + wy, z + wz);
    }
    ...
    public Vec3 getBonePosition(String boneName) {
        // For rider anchor, always use deterministic server-side math to avoid client/server divergence
        if ("rider".equals(boneName)) {
    ...
    private Vec3 getClientBonePosition(String boneName) {
        try {
            ...
            if ("beam_origin".equals(boneName) && beamOriginPos != null) {
                return beamOriginPos;
            }
            if ("rider".equals(boneName) && riderAnchorPos != null) {
                return riderAnchorPos;
            }
            return null;
        } catch (Exception e) {
            return null;
    ...
    private Vec3 getServerBonePosition(String boneName) {
        if ("beam_origin".equals(boneName)) {
            ...
            // Defer to getMouthPosition() math to keep consistent; returning null lets fallback handle it
            return null;
        }
        if ("rider".equals(boneName)) {
            // Server-side fallback using provided default-pose pivot (pixels): X=0, Y=7.95, Z=-12.15
            double localX = (0.0 / 16.0) * MODEL_SCALE;
            double localY = (7.95 / 16.0) * MODEL_SCALE;
            double localZ = (-12.15 / 16.0) * MODEL_SCALE;

            double radians = Math.toRadians(-yBodyRot);
            double rotatedX = localX * Math.cos(radians) - localZ * Math.sin(radians);
            double rotatedZ = localX * Math.sin(radians) + localZ * Math.cos(radians);

            return new Vec3(getX() + rotatedX, getY() + localY, getZ() + rotatedZ);
        }
        return null;
    }

    // (Synced rider anchor removed; use deterministic server placement)

    // (Client-only locator helper removed; renderer tracks needed positions.)
    ...
    // (Rider attacking flag removed; not referenced elsewhere)
    ...
    // (beam_end timing removed with synced ability type)
    ...
    // (Per-bit control mutator removed.)

    // (Per-bit mutators removed; unified via controlState byte.)
    ...
    // (isBeingRidden/flight speed helpers removed; not referenced.)
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            ...
            Vec3 synced = getBeamEndPosition();
            this.clientBeamEnd = synced;

            this.prevClientBeamStart = this.clientBeamStart;
            Vec3 s2 = getBeamStartPosition();
            this.clientBeamStart = s2;
        }
    ...
    private void playCustomAmbientSound() {
        ...
        if (isAggressive() || isBeaming() || getActiveAbility() != null) {
            return;
        }
    ...
    public boolean isFlightControllerStuck() {
        if (this.moveControl instanceof com.leon.lightningdragon.server.ai.navigation.DragonFlightMoveHelper flightHelper) {
    ...
    // (Legacy flight-helper query removed.)
    ...
    public int getMaxHeadYRot() {
        return 180; // large horizontal head sweep
    }

    @Override
    public int getHeadRotSpeed() {
        return 180; // faster head turn speed (vanilla zombies use ~30)
    }
    ...
    protected void registerGoals() {
        // Bring back full goal stack and use DragonMeleeAttack for ground combat
        ...
        this.goalSelector.addGoal(6, new DragonCombatFlightGoal(this));   // Flight decision for combat
        this.goalSelector.addGoal(7, new DragonAirCombatGoal(this));      // Air combat (when flying)
        this.goalSelector.addGoal(8, new com.leon.lightningdragon.server.ai.goals.DragonMeleeAttackGoal(this, 1.35, true)); // Ground melee combat
    ...
    public void playAnimation(RawAnimation animation) {
        ...
        if (!animation.getAnimationStages().isEmpty()) {
            ...
            // Convert full animation name to a trigger key if registered (e.g., "...lightning_bite" -> "lightning_bite")
            String key = animationName;
            int idx = animationName.lastIndexOf('.');
            if (idx >= 0 && idx < animationName.length() - 1) {
                key = animationName.substring(idx + 1);
            }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        // Use standard AnimationController for reliable keyframe + loop handling
        ...
        AnimationController<LightningDragonEntity> bankingController =
                new AnimationController<>(this, "bankingController", 12, this::bankingPredicate);
        ...
        // Beam animations
        actionController.triggerableAnim("beam_start",
                RawAnimation.begin().thenPlay("animation.lightning_dragon.beam_start"));
        actionController.triggerableAnim("lightning_beam",
                RawAnimation.begin().thenPlay("animation.lightning_dragon.lightning_beam"));
        actionController.triggerableAnim("beam_end",
                RawAnimation.begin().thenPlay("animation.lightning_dragon.beam_end"));
    ...
    private void registerVocalTriggers(AnimationController<LightningDragonEntity> action) {
        String[] keys = new String[] {
                ...
                "growl_warning","roar","hurt","lightning_bite","dodge"
        };
        for (String key : keys) {
    ...
    //PREDICATES
    // Removed walk/run and flight switch predicates; see movement controller and animation.json
    ...
    public double getCachedDistanceToOwner() {
        if (tickCount - ownerDistanceCacheTime >= 5) {
    ...
    public List<Projectile> getCachedNearbyProjectiles() {
        if (tickCount - nearbyProjectilesCacheTime >= 3) {
            cachedNearbyProjectiles = DragonMathUtil.getEntitiesNearby(this, Projectile.class, 30.0);
