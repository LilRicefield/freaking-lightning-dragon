...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob {
    ...
    // Controller-blend values removed; animation.json drives blending directly
    ...
    // (Rider control accessors and control state handling are defined later in this class.)

    // ===== ABILITY SYSTEM METHODS =====
    public void tryUseRangedAbility() {
        return;
    }
    ...
    // Ability activation entrypoints (server-side)
    public void sendAbilityMessage(Object abilityType) {
        if (abilityType instanceof DragonAbilityType<?, ?> at) {
            //noinspection unchecked
            this.tryActivateAbility((DragonAbilityType) at);
            return;
        }
        if (abilityType instanceof String s) {
            useRidingAbility(s);
        }
    }
    public void useRidingAbility(String abilityName) {
        ...
        if (type != null) {
            //noinspection unchecked
            this.tryActivateAbility((com.leon.lightningdragon.server.entity.ability.DragonAbilityType) type);
        }
    }
    ...
    // Helper to trigger dragon animation from other systems
    public void playDragonAnimation(RawAnimation animation) {
        playAnimation(animation);
    }

    public DragonAbilityType<?, ?>[] getDragonAbilities() {
        return new DragonAbilityType[]{ ModAbilities.BITE };
    }
    ...
    protected void registerGoals() {
        ...
        // Combat goals - staggered priorities to avoid thrash
        this.goalSelector.addGoal(4, new DragonCombatFlightGoal(this));  // Takes flight for combat
        this.goalSelector.addGoal(5, new DragonAirCombatGoal(this));     // Air combat (when flying)
        this.goalSelector.addGoal(6, new DragonGroundCombatGoal(this));  // Ground combat (when grounded)
    ...
    // Removed enhanced controller hooks; standard controller handles blending and timing
