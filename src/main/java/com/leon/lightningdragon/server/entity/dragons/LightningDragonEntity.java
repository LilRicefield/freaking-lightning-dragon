...
public class LightningDragonEntity extends DragonEntity implements FlyingAnimal, RangedAttackMob {
    ...
    // Post-load stabilization to preserve mid-air riding state across save/load
    ...
    public void useRidingAbility(String abilityName) {
        ...
        if (!(cp instanceof net.minecraft.world.entity.LivingEntity)) {
            System.out.println("[LD][ABILITY] Blocked (no rider) dragonId=" + getId() + " ability=" + abilityName);
            return;
        }
        if (this.isTame() && cp instanceof net.minecraft.world.entity.player.Player p && !this.isOwnedBy(p)) {
            System.out.println("[LD][ABILITY] Blocked (not owner) dragonId=" + getId() + " rider=" + p.getGameProfile().getName());
            return; // owner-gate abilities on tamed dragons
        }
        var type = AbilityRegistry.get(abilityName);
        if (type != null) {
            System.out.println("[LD][ABILITY] Start dragonId=" + getId() + " tamed=" + this.isTame() +
                    " rider=" + (cp instanceof net.minecraft.world.entity.player.Player pr ? pr.getGameProfile().getName() : "?") +
                    " ability=" + abilityName);
    ...
    // (Head-follow rider anchor removed; seating uses static body-based offsets)
    ...
    public int getAttackKind() { return this.entityData.get(DATA_ATTACK_KIND); }
    public void setAttackKind(int kind) { this.entityData.set(DATA_ATTACK_KIND, kind); }

    public int getAttackPhase() { return this.entityData.get(DATA_ATTACK_PHASE); }
    ...
    public void tick() {
        ...
        if (!level().isClientSide && postLoadAirStabilizeTicks > 0) {
            ...
            // Give a tiny buoyancy to counter immediate drop before rider inputs arrive
            var v = getDeltaMovement();
            if (v.y < 0.02) setDeltaMovement(v.x * 0.98, 0.02, v.z * 0.98);

            postLoadAirStabilizeTicks--;
        }

        // Decrement rider takeoff window
        if (!level().isClientSide && riderTakeoffTicks > 0) {
            riderTakeoffTicks--;
        }

        if (actionAnimWindow > 0) actionAnimWindow--;
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(8, new com.leon.lightningdragon.server.ai.goals.DragonMeleeAttackGoal(this)); // Ground melee combat
    ...
    public void playAnimation(RawAnimation animation) {
        ...
        if (!animation.getAnimationStages().isEmpty()) {
            ...
            System.out.println("[LD][ANIM] triggerAnim entity=" + getId() + " key=" + key);
            triggerAnim("action", key);
        }
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        ...
        AnimationController<LightningDragonEntity> pitchingController =
                new AnimationController<>(this, "pitchingController", 12, this::pitchingPredicate);
    ...
    private PlayState pitchingPredicate(AnimationState<LightningDragonEntity> state) {
        ...
        if (this.isVehicle() && this.getControllingPassenger() instanceof Player) {
            // Ridden dragon: Only pitch based on rider input (Spacebar/L-Alt)
            if (isGoingUp()) {
            ...
            // Wild dragon: Use pitch changes for natural flight movement
            ...
            if (pitchSmoothedPitch > enter) desiredDir = 1;   // Pitching down (positive pitch)
            else if (pitchSmoothedPitch < -enter) desiredDir = -1;  // Pitching up (negative pitch)
    ...
    private PlayState actionPredicate(AnimationState<LightningDragonEntity> state) {
        // Only allow action animations to run when this entity has explicitly opened the window
        return (actionAnimWindow > 0) ? PlayState.CONTINUE : PlayState.STOP;
    }
