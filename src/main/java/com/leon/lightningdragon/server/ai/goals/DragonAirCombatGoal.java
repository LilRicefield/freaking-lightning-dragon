...
public class DragonAirCombatGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        return t != null && t.isAlive() && dragon.distanceToSqr(t) <= ENGAGE_MAX_RANGE_SQR;
    }
    ...
    private void handleClimb(LivingEntity target) {
        ...
        double desiredY = com.leon.lightningdragon.util.DragonMathUtil.clampAltitude(target.getY() + 8.0, groundLevel, 8, 22);
    ...
    private void doDiveBomb(LivingEntity target) {
        ...
        if (phaseTimer < windup) {
            ...
            com.leon.lightningdragon.util.DragonMathUtil.smoothLookAt(dragon, target, 25f, 25f);
        ...
        if (phaseTimer < windup + commit) {
            ...
            com.leon.lightningdragon.util.DragonMathUtil.smoothLookAt(dragon, target, 35f, 35f);
            dragon.setAttackKind(ATTACK_KIND_DIVE);
            ...
            if (dragon.distanceTo(target) <= 3.2 && com.leon.lightningdragon.util.DragonMathUtil.hasLineOfSight(dragon, target)) {
    ...
    private void doRangedCircle(LivingEntity target) {
        ...
        float speed = 0.05f;
        Vec3 circle = com.leon.lightningdragon.util.DragonMathUtil.circleEntityPosition(target, radius, speed, circleClockwise, phaseTimer, 0);
        Vec3 goal = circle.add(0, 10, 0);
        goal = ensureClearArc(goal, 8);
        dragon.getMoveControl().setWantedPosition(goal.x, goal.y, goal.z, 1.0);
        com.leon.lightningdragon.util.DragonMathUtil.smoothLookAt(dragon, target, 20f, 20f);

        float yawErr = com.leon.lightningdragon.util.DragonMathUtil.yawErrorToTarget(dragon, target);
        boolean los = com.leon.lightningdragon.util.DragonMathUtil.hasLineOfSight(dragon, target);
