...
public class DragonFlightGoal extends Goal {
    ...
    private BlockPos findGroundLevel(BlockPos startPos) {
        int groundY = dragon.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, startPos.getX(), startPos.getZ());
        return new BlockPos(startPos.getX(), groundY, startPos.getZ());
    }
}
